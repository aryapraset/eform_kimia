...
export const createCorrectTables = async () => {
  try {
    ...
    const { error: createChemicalsError } = await supabase.rpc(
    ...
    const { error: createUsageLogsError } = await supabase.rpc(
    ...
    const { error: createStockHistoryError } = await supabase.rpc(
      "create_stock_history_table"
    );

    if (createStockHistoryError) {
      console.error(
        "Error creating stock_history table:",
        createStockHistoryError
      );
